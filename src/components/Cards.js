  ...
  return (
    <Card maxW='xs' minW='xs' className='one-card' variant={'outline'} style={{ boxShadow: "rgba(149, 157, 165, 0.2) 0px 8px 24px" }} >
      <CardBody>
        ...
        <Heading size='md' style={{ display: 'flex', alignItems:'center', justifyContent: 'space-between' }} >{data.title.length > 37 ? `${data.title.substring(0, 37)}...` : data.title}<FaHandsHelping color={'tomato'} /></Heading>
